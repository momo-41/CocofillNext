...
const TestCreateShiftView = ({ month, week }: CreateShiftViewProps) => {
  ...
  useEffect(() => {
    // 特定の月と週に基づいた週データを設定
    const startOfWeek = calcMonthWeek(month, week); // 特定の月と週の最初の日を計算
    setCurrentWeek(calcWeek(startOfWeek.format("YYYY-MM-DD"))); // calcWeekを使って該当週の日付リストを生成
  }, [month, week]);

  const moveWeek = (type: string) => {
    const startDay =
      type === "add"
        ? currentWeek[0].add(7, "day").format("YYYY-MM-DD")
        : currentWeek[0].subtract(7, "day").format("YYYY-MM-DD");
    setCurrentWeek(calcWeek(startDay));
    setWeekKey((prevKey) => prevKey + 1);

    // 現在のmonthとweekを基に次の週番号を計算
    const newWeek = type === "add" ? week + 1 : week - 1;

    // URLを更新
    router.push(`/create-shift/${month}/${newWeek}`);
  };
  ...
  const handleUpdate = () => {
    const startDate = currentWeek[0].format("YYYY-MM-DD"); // 週の初めの日付
    const endDate = currentWeek[currentWeek.length - 1].format("YYYY-MM-DD"); // 週の終わりの日付
